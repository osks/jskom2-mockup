	...
	{#if $activeConnection}
		<div class="relative">
			...
			{#if menuOpen}
				...
				<div class="absolute bottom-full left-0 right-0 z-40 mb-2 rounded-2xl bg-surface-1 py-1.5 shadow-[0_2px_8px_rgba(0,0,0,0.08)] border border-surface-4">
						<!-- Other connections -->
						{#if $connections.length > 1}
							{#each $connections as conn}
								...
								<button
									...
									class="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm transition-colors {active ? 'text-txt font-medium' : 'text-txt-secondary'} active:bg-surface-2"
								>
									<div class="min-w-0 flex-1">
						...
						<button
							...
							class="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-txt-secondary active:bg-surface-2"
						>
