			...
			{#if activeText}
				<div class="relative">
					...
					{#if moreMenuOpen}
						...
						<div class="absolute bottom-full left-0 z-40 mb-2 min-w-48 rounded-2xl bg-surface-1 py-1.5 shadow-md border border-surface-4">
...
{#if showReviewInput}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20" onclick={closeReviewInput}>
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div class="mx-6 w-full max-w-xs rounded-2xl bg-surface-1 p-4 shadow-md border border-surface-4" onclick={(e) => e.stopPropagation()}>
