...
<div class="relative z-10">
		...
		<div class="flex h-14 items-center gap-2.5 px-4">
			...
			<div class="flex min-w-0 flex-1 flex-col items-center justify-center">
				{#if $pageTitle}
					<span class="max-w-full truncate text-sm font-medium text-txt">{$pageTitle}</span>
				{/if}
				{#if $pageSubtitle}
					<span class="max-w-full truncate text-[11px] text-txt-secondary -mt-0.5">{$pageSubtitle}</span>
				{/if}
			</div>
